...
impl Node{
    ...
    fn take_largest_out(&self,child_index:usize){
...
// go through values, delete
impl Node{
    pub fn get_all(&self) -> Vec<EntryElement> {
        let mut result: Vec<EntryElement> = Vec::new();

        for i in 0..self.elements.len() {
            if i < self.children.len() {
                if let Some(ref child_node) = self.children[i] {
                    result.extend(child_node.get_all());
                }
            }
            result.push(self.elements[i].clone());
        }
        if self.children.len() > self.elements.len() {
            if let Some(ref last_child_node) = self.children[self.elements.len()] {
                result.extend(last_child_node.get_all());
            }
        }

        result
    }
    pub fn delete(&mut self, key_attribute:&str){
        let key=extract(key_attribute).unwrap();

        for i in 0..self.elements.len(){
            let mut e=self.elements[i].clone();
            if e.extract_number_from_key().unwrap()==key{
                e.tombstone=true;
            ...
            if e.extract_number_from_key().unwrap()>key{
                ...
                child.delete(key_attribute);
                self.children[i]= Option::from(child);
                return;
            }
        }
        // last child
        let ind=self.elements.clone().len();
        let mut child =self.children[ind].clone().unwrap().clone().deref().clone();
        child.delete(key_attribute);
        self.children[ind]= Option::from(Box::new(child));
        return;
    }
}